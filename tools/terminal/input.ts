...
import { EventType, Key, KeyboardEvent, KeyPressAction, TerminalEvent } from './event.ts';
...
const escapeCodeToEvent = new Map<string, TerminalEvent>([
    ...
    ['\x7f', createKeyboardEvent(Key.Backspace, KeyPressAction.Down, false, false)],
    ...
    [`${ESC}[1;5A`, createKeyboardEvent(Key.Up, KeyPressAction.Down, false, true)],
    [`${ESC}[1;5B`, createKeyboardEvent(Key.Down, KeyPressAction.Down, false, true)],
    [`${ESC}[1;5C`, createKeyboardEvent(Key.Right, KeyPressAction.Down, false, true)],
    [`${ESC}[1;5D`, createKeyboardEvent(Key.Left, KeyPressAction.Down, false, true)],
]);

export async function detectTerminalEvent(inputStream: IReader): Promise<TerminalEvent | undefined> {
