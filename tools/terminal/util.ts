 ...
 *
 * @param {IWriter} outputStream An object implementing IWriter
 * @param {string} text Text to be written to the stream
 */
export async function writeToStream(outputStream: IWriter, text: string) {
    const contentBytes = new TextEncoder().encode(text);
    let bytesWritten = 0;

    // Iwriter.write doesn't write the whole buffer at once, that's why we need to loop it
    while (bytesWritten < contentBytes.byteLength) {
        bytesWritten += await outputStream.write(contentBytes.subarray(bytesWritten));
    }
}
 ...
 */
export function resizeArray<T>(array: T[], newLength: number, newItem: T | null = null) {
    const deleteCount = Math.max(0, array.length - newLength);
    const newItems = deleteCount === 0 ? Array(newLength - array.length).fill(newItem) : [];
 ...
 * @param {T} newItem         Item to fill new slots with
 */
export function resizeNestedArray<T>(array: T[][], newWidth: number, newHeight: number, newItem: T | null = null) {
    // Remove/add rows
...
export function isInsideRectangle(width: number, height: number, x: number, y: number) {
